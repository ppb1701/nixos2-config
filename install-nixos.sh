...
echo "    - Collabora      (Document editing - requires Nextcloud)"
...
echo ""
echo "═══════════════════════════════════════════════════════════════════════════"
echo "📝 CONFIGURE: Collabora Online (Document Editing for Nextcloud)"
echo "═══════════════════════════════════════════════════════════════════════════"
echo ""
echo "Collabora Online provides LibreOffice-based document editing inside"
echo "Nextcloud, replacing Google Docs or Office 365."
echo ""
echo "⚠️  IMPORTANT: Collabora requires the nixos-unstable channel."
echo "   The system must be on unstable for the collabora-online package."
echo "   Home Manager must also use the master branch (not a release branch)."
echo "   See docs/TROUBLESHOOTING.md for channel management and pinning tips."
echo ""
echo "Collabora status:"
echo "  - nixos2 (this server): DISABLED (standby/failover)"
echo "  - Primary server (nixos-config): ENABLED"
echo ""
echo "⚠️  PREREQUISITE: Nextcloud MUST be running and accessible before"
echo "   configuring Collabora. Complete the Nextcloud setup above first!"
echo ""
echo "To configure Collabora (after Nextcloud is running):"
echo ""
echo "  1. Add DNS rewrite in AdGuard Home:"
echo "     Go to Filters → DNS rewrites, add:"
echo "     collabora.home → YOUR_IP"
echo ""
echo "  2. Enable Collabora in services.nix:"
echo "     Set services.collabora-online.enable = true;"
echo "     sudo nixos-rebuild switch"
echo ""
echo "  3. Open Nextcloud admin panel:"
echo "     http://nextcloud.home:8280/settings/admin/richdocuments"
echo ""
echo "  4. Select 'Use your own server' and enter:"
echo "     http://collabora.home"
echo ""
echo "  5. Scroll down to 'Allow list for WOPI requests' and add:"
echo "     127.0.0.1"
echo ""
echo "  6. Click Save"
echo ""
echo "  7. Test: Create or open a .docx/.xlsx/.pptx file in Nextcloud"
echo "     It should open in the Collabora editor within the browser."
echo ""
echo "Useful aliases (service name is coolwsd, NOT collabora-online):"
echo "  cos   - Collabora status"
echo "  cor   - Collabora restart"
echo "  col   - Collabora logs"
echo ""
echo "To disable Collabora:"
echo "  Set services.collabora-online.enable = false; in services.nix"
echo ""
echo ""
echo "═══════════════════════════════════════════════════════════════════════════"
...
echo ""
echo "═══════════════════════════════════════════════════════════════════════════"
echo "🖥️  CONFIGURE: Samba + Time Machine (macOS Backup)"
echo "═══════════════════════════════════════════════════════════════════════════"
echo ""
echo "Samba and Time Machine sharing are ENABLED on this server (primary instance)."
echo "Configuration is in modules/timemachine.nix."
echo ""
echo "REQUIRED - Set the Samba password for tmuser (one-time, survives rebuilds):"
echo "  sudo smbpasswd -a tmuser"
echo ""
echo "After setting the password, Macs can find and use this server for backups:"
echo "  - System Settings → General → Time Machine → Add Backup Disk"
echo "  - The server appears automatically via mDNS/WSDD (samba-wsdd)"
echo "  - Or connect manually: Finder → Go → Connect to Server → smb://nixos2.local"
echo "  - Authenticate with tmuser and the password you set above"
echo ""
echo "Share details:"
echo "  Path:  /mnt/nextcloud-data/timemachine"
echo "  Quota: 1500G (fruit:time machine max size)"
echo ""
echo "Service management:"
echo "  systemctl status samba        # Samba daemon"
echo "  systemctl status samba-wsdd   # WS-Discovery (Finder auto-discovery)"
echo "  journalctl -u samba -f        # Samba logs"
echo ""
echo "To disable Time Machine sharing:"
echo "  Set services.samba.enable = false; and services.samba-wsdd.enable = false;"
echo "  in modules/timemachine.nix, then rebuild."
echo ""
echo ""
echo "═══════════════════════════════════════════════════════════════════════════"
